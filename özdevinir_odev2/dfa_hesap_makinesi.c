...
int main(void)
{
	...
	//Yazýlacak bir dosya açar (output).
	if ((output = fopen("output.txt", "w+")) == NULL)
			printf("It was not possible to open the file to write!");	
			

	//Girdi dosyasýndan satýr satýr okur.
	while (fgets(buff, sizeof(buff), input))
	{		
		numTokens = -1;
		
		printf("\n------------------------------------------------------\n");
		printf("GÝRÝLEN ÝFADELER: %s", buff); //Orjinal giriþi ekrana yazdýrýr.
		fprintf(output, "\n------------------------------------------------------\n"); 
		fprintf(output, "GÝRÝLEN ÝFADELER: %s\n", buff);  //prints the original input to the output file.		
...
//Boþ alanlarý bir dizeden siler.
void deleteBlankSpaces(char* str)
{
	const char* d = str;
	int i = 0;
...
//Baðýmsýz deðiþken bir sayý ise '1' deðerini döndürür.
int isItNumber(char n) 
{
...
//Baðýmsýz deðiþken bir iþleç ise '1' deðerini döndürür.
...
//TAMSAYI(Ýnteger) mý yoksa kayan nokta (Float) mý olduðunu kontrol eder.
void DFARecognizer(char* exp, char tokens[][nLenghtNameTokens], int *numTokens)
{
	
  // Ýlk sütun, bir iþleci tüketen geçiþlerle ilgilidir (+, -, / veya *).
 // Ýkinci sütun, sol parantez kullanan geçiþlerle ilgilidir.
 // Üçüncü sütun, sað parantez kullanan geçiþlerle ilgilidir.
 // Dördüncü sütun, bir sayýyý tüketen geçiþlerle ilgilidir.
 // Beþinci sütun, bir noktayý tüketen geçiþlerle ilgilidir.
 // Altýncý sütun, baþka herhangi bir karakteri tüketen geçiþlerle ilgilidir.
	...
	//Hangi öðe, bir durumun kabul durumu olup olmadýðýný gösterir.
	...
	while ((exp[inputPosition] != '\n'))
	{
		...
		//Geçiþten sonraki yeni durumun bir kabul durumu olup olmadýðýný doðrular.
		...
		switch(lastAcceptedState)
		{
			...
			case 3:
			{
				strcpy(tokens[*numTokens], "Sað-Parantez");
				break;
			}
			case 4:
			{
